                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <div class="grid grid-cols-12 gap-4">
                        ...
                        <div class="col-span-12 md:col-span-12 xl:col-span-12">
                            <form method="POST" action="{{ url('paper_Allocation/'.$paper_Allocation->id) }}" onsubmit="return validateForm()">
                                @csrf
                                @method('PUT')
                                 ...
                                 <div class="relative mb-6" >
                                    ...
                                    @foreach($papers as $paper)
                                        <option value="{{$paper->id}}"  {{ $paper->id == $paper_Allocation->paper_id ? 'selected':''}}>{{$paper->exam_paper_id.':'.$paper->subject->code.'-'.$paper->subject->name}}</option>
                                    ...
                                    @enderror
                                    <label data-te-select-label-ref>Select Paper</label>
                                    </div>
                                    ...
                                    <select data-te-select-init data-te-select-filter="true"
                                    class="@error('teacher_id') is-invalid @enderror peer block min-h-[auto] w-full rounded border-0 bg-transparent px-3 py-[0.32rem] leading-[1.6] outline-none transition-all duration-200 ease-linear focus:placeholder:opacity-100 data-[te-input-state-active]:placeholder:opacity-100 motion-reduce:transition-none dark:text-neutral-200 dark:placeholder:text-neutral-200 [&:not([data-te-input-placeholder-active])]:placeholder:opacity-0"
                                    id="teacher_id"
                                    name="teacher_id"
                                    placeholder="Teacher"
                                    >
                                    ...
                                    @foreach($teachers as $teacher)
                                        <option value="{{$teacher->id}}"  {{ $teacher->id == $paper_Allocation->teacher_id ? 'selected':''}}>{{$teacher->name_prefix.' '.$teacher->name.'('.$teacher->department->code.'-'.$teacher->department->name.')'}}</option>
                                    ...
                                    @enderror
                                    <label data-te-select-label-ref>Select Teacher</label>
                                    </div>
                                ...
                                class="mb-6 flex min-h-[1.5rem] pl-[1.5rem]">
                                    ...
                                    <input
                                        ...
                                        name="status"
                                        id="status" 
                                        {{$paper_Allocation->status?'checked':''}}
                                        />
                                    <label
