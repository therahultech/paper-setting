                    ...
                    <table class="min-w-full text-left text-sm font-light nowrap" id='view_table' style="width:100%">
                        <thead class="border-b font-medium dark:border-neutral-500">
                            <tr class="border-b dark:border-neutral-500">
                                ...
                                <th scope="col" class="px-6 py-4">Is Annual</th>
                        ...
                        <tbody>
                            ...
                            <tr class="border-b dark:border-neutral-500">
                                ...
                                <td class="whitespace-nowrap px-6 py-4">{{$course->is_year_based==1?'Yes':'No'}}</td>
                                <td class="whitespace-nowrap px-6 py-4">{{$course->duration}}</td>
                                <td class="whitespace-nowrap px-6 py-4">{{$course->status==1?'Active':'Inactive'}}</td>
                                <td class="whitespace-nowrap px-6 py-4">
                                    @can('course-edit')
                                    <a href="{{ url('course/'.$course->id.'/edit') }}" class="inline-block float-left"><i class="fa-solid fa-pen-to-square px-4"></i></a> 
                                    @endcan

                                    @can('course-delete')

                                    <form action="{{url('course/'.$course->id)}}" method="POST" onsubmit="return validateDelete()">
                                        @csrf
                                        @method('DELETE')
