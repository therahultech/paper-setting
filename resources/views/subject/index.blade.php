                    ...
                    <table class="min-w-full text-left text-sm font-light nowrap" id='view_table' style="width:100%">
                        <thead class="border-b font-medium dark:border-neutral-500">
                            <tr class="border-b dark:border-neutral-500">
                                ...
                                <th scope="col" class="px-6 py-4">Is Credit Based</th>
                        ...
                        <tbody>
                            @foreach($subjects as $ckey=>$subject)
                            <tr class="border-b dark:border-neutral-500">
                                <td class="whitespace-nowrap px-6 py-4 font-medium">{{$ckey+1}}</td>
                                <td class="whitespace-nowrap px-6 py-4">{{$subject->code}}</td>
                                <td class="whitespace-nowrap px-6 py-4">{{$subject->name}}</td>
                                <td class="whitespace-nowrap px-6 py-4">{{$subject->is_credit_based==1?'Yes':'No'}}</td>
                                <td class="whitespace-nowrap px-6 py-4">{{$subject->status==1?'Active':'Inactive'}}</td>
                                <td class="whitespace-nowrap px-6 py-4">
                                    @can('subject-edit')
                                    <a href="{{ url('subject/'.$subject->id.'/edit') }}" class="inline-block float-left"><i class="fa-solid fa-pen-to-square px-4"></i></a> 
                                    @endcan

                                    @can('subject-delete')

                                    <form action="{{url('subject/'.$subject->id)}}" method="POST" onsubmit="return validateDelete()">
                                        @csrf
                                        @method('DELETE')
