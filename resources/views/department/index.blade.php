            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <table class="min-w-full text-left text-sm font-light nowrap" id='view_table' style="width:100%">
                        ...
                        <tbody>
                            @foreach($depts as $dkey=>$dept)
                            <tr class="border-b dark:border-neutral-500">
                                <td class="whitespace-nowrap px-6 py-4 font-medium">{{$dkey+1}}</td>
                                <td class="whitespace-nowrap px-6 py-4">{{$dept->code}}</td>
                                <td class="whitespace-nowrap px-6 py-4">{{$dept->name}}</td>
                                <td class="whitespace-nowrap px-6 py-4">{{$dept->status==1?'Active':'Inactive'}}</td>
                                <td class="whitespace-nowrap px-6 py-4">
                                    @can('department-edit')
                                    <a href="{{ url('department/'.$dept->id.'/edit') }}" class="inline-block float-left"><i class="fa-solid fa-pen-to-square px-4"></i></a> 
                                    @endcan

                                    @can('department-delete')

                                    <form action="{{url('department/'.$dept->id)}}" method="POST" onsubmit="return validateDelete()">
                                        @csrf
                                        @method('DELETE')
